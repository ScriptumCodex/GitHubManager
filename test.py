...
def check_base_path():
    ...
    if os.path.exists(base_path):
        ...
        return True
    # create directory/folder where the porjects will be backed
    # up in case the folder does not exists already
    else:
...
def report_orgs(option):
    ...
    if (option == "-f") or (option == "all" and src is not None):
        str(src)
    ...
    for org in g.get_user(user).get_orgs():
        ...
        for repo in org.get_repos():
            ...
            if os.path.exists(base_path + "/" + org.login + "/" + repo.name):
                ...
                check_base_path()
                if not os.path.exists(base_path + "/" + org.login):
